...
export default {
  ...
  run(item) {
    ...
    if (this.terminal === 'iTerm (external, macOS)')
      return this.runOnIterm(directory, item);
    if (this.terminal === 'Terminal (external, macOS)')
      return this.runOnTerminal(directory, item);
    if (this.terminal === 'platformio-ide-terminal (Atom)')
  ...
  runOnIterm(directory, item) {
    this.asyncExec(`osascript \
      ...
      -e     'write text "cd ${directory}"' \
      -e     'write text "${item.command}" ${item.execute ? '' : 'newline NO'}' \
      -e   'end tell' \
      -e 'end tell'
    `);
  },
  ...
  async updateCommands() {
    const commands = await this.asyncExec('taito -q descriptions');
    return commands.split(/\r?\n/).map(c => {
      ...
      return {
        ...
        focus: split[0].indexOf('&focus') !== -1 ||
          /[A-Z]/.test(displayName), // Uppercase letters -> has params
        execute: !/[A-Z]/.test(displayName),
      ...
      return {
        displayName: command,
        command: command,
        focus: /[A-Z]/.test(command),
        execute: !/[A-Z]/.test(command),
